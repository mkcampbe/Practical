...
fiber_df <- sample_df %>% 
  ...
  filter(enterotype == "Type 1",
         study_week == "week1",
         supplement_consume d == "BRMPS" | supplement_consumed == "LOODAT") %>% 
  drop_na(fiber_g)
# average age and sd of those who consumed LOODAT 
...
bartlett.test(avg_richness ~ enterotype, data = brmps_1x_df) #p-value = 0.01464 < 0.05. so variances are not good 
...
brmps1_ent3 <- brmps_1x_df %>% 
  ...
  shapiro.test(brmps1_race3$avg_richness) #p-value = 0.7939
  
brmps1_ent4 <- brmps_1x_df %>% 
  filter(enterotype == "Type 4")
  shapiro.test(brmps1_ent4$avg_richness) #p-value = 0.9148
